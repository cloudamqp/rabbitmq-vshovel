  ...
  with_ch(Config,
          fun(Ch) ->
            set_param(Config,
                      <<"test">>, [{<<"src-queue">>, <<"src">>},
                                   {<<"dest-queue">>, <<"dest">>}]),
            %% The catch is because connections link to the vshovel,
            %% so one connection will die, kill the vshovel, kill
...
security_validation_add_user() ->
  [begin
     rabbit_vhost:add(U, <<"rabbit-cli">>),
     rabbit_auth_backend_internal:add_user(U, <<>>, <<"rabbit-cli">>),
...
security_validation_remove_user() ->
  [begin
     rabbit_vhost:delete(U, <<"rabbit-cli">>),
     rabbit_auth_backend_internal:delete_user(U, <<"rabbit-cli">>)
   end || U <- [<<"a">>, <<"b">>]],
  ok.
...
set_param(Config, Name, Value) ->
  ...
  await_vshovel(Config, Name).

set_param_nowait(Config, Name, Value) ->
  ok = rabbit_ct_broker_helpers:rpc(Config, 0,
                                    rabbit_runtime_parameters, set, [
                                      <<"/">>, <<"vshovel">>, Name, [{<<"src-uri">>, <<"amqp://">>},
                                                                     {<<"dest-uri">>, [<<"amqp://">>]} |
                                                                     Value], none]).
...
valid_param1(_Config, Value, User) ->
  ok = rabbit_runtime_parameters:set(
    <<"/">>, <<"vshovel">>, <<"a">>, Value, User),
  ok = rabbit_runtime_parameters:clear(<<"/">>, <<"vshovel">>, <<"a">>, <<"rabbit-cli">>).
...
clear_param(Config, Name) ->
  rabbit_ct_broker_helpers:rpc(Config, 0,
                               rabbit_runtime_parameters, clear, [<<"/">>, <<"vshovel">>, Name, <<"rabbit-cli">>]).
...
cleanup1(_Config) ->
  ...
  [rabbit_amqqueue:delete(Q, false, false, <<"rabbit-cli">>) || Q <- rabbit_amqqueue:list()].

await_vshovel(Config, Name) ->
  rabbit_ct_broker_helpers:rpc(Config, 0,
                               ?MODULE, await_vshovel1, [Config, Name]).

await_vshovel1(_Config, Name) ->
  await(fun() -> lists:member(Name, vshovels_from_status()) end).
...
await_autodelete1(_Config, Name) ->
  await(fun() -> not lists:member(Name, vshovels_from_parameters()) end),
  await(fun() -> not lists:member(Name, vshovels_from_status()) end).
...
vshovels_from_status() ->
  S = rabbit_vshovel_status:status(),
  [N || {{<<"/">>, N}, dynamic, {running, _}, _} <- S].

vshovels_from_parameters() ->
  L = rabbit_runtime_parameters:list(<<"/">>, <<"vshovel">>),
