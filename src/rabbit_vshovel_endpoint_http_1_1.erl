...
handle_response(HTTPErrCode, Channel, DeliveryTag) when HTTPErrCode >= ?HTTP_200_RANGE,
                                                        HTTPErrCode <  ?HTTP_300_RANGE ->
    %% Issue 'basic.ack' back to the broker for 2XX response codes.
    amqp_channel:cast(Channel, #'basic.ack'{ delivery_tag=DeliveryTag }),
    rabbit_event:notify(vshovel_result, [{source, ?MODULE}, {result, HTTPErrCode}]),
    ok;

%% HTTP 300 Range Response Codes
handle_response(HTTPErrCode, Channel, DeliveryTag) when HTTPErrCode >= ?HTTP_300_RANGE,
...
handle_response(HTTPErrCode, Channel, DeliveryTag) when HTTPErrCode >= ?HTTP_400_RANGE,
...
handle_response(HTTPErrCode, Channel, DeliveryTag) when HTTPErrCode >= ?HTTP_500_RANGE,
...
parse_http_options([{Tag = ssl, Value} | Args], Acc) ->
    parse_http_options(Args, [{Tag, parse_ssl_options(Value)}|Acc]);
...
parse_ssl_options(SSLOpts) ->
    parse_ssl_options([{?TO_ATOM(Opt), V} || {Opt, V} <- SSLOpts], []).

parse_ssl_options([], Acc) -> Acc;
parse_ssl_options([{Tag = verify, Value} | Args], Acc) ->
    parse_ssl_options(Args, [{Tag, ?TO_ATOM(Value)}|Acc]);
...
parse_ssl_options([{Tag = depth, Value} | Args], Acc) ->
    parse_ssl_options(Args, [{Tag, ?TO_INTEGER(Value)}|Acc]);
parse_ssl_options([{Tag = cert, Value} | Args], Acc) ->
    parse_ssl_options(Args, [{Tag, Value}|Acc]);
parse_ssl_options([{Tag = certfile, Value} | Args], Acc) ->
    parse_ssl_options(Args, [{Tag, ?TO_LIST(Value)}|Acc]);
...
parse_ssl_options([{Tag = cb_info, Value} | Args], Acc) ->
    parse_ssl_options(Args, [{Tag, Value}|Acc]);
parse_ssl_options([_|Args], Acc) -> parse_ssl_options(Args, Acc).
